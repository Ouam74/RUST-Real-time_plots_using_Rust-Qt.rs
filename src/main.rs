...
use qt_core::{
    q_init_resource, qs, slot, CheckState, QBox, QObject, QPtr, QPointF, SlotNoArgs, QFlags,
    AlignmentFlag, QCoreApplication, ApplicationAttribute, q_event_loop::ProcessEventsFlag
};
...
impl TodoWidget {
    ...
    unsafe fn init(self: &Rc<Self>) {
       ...
       self.chart.set_background_brush(&QBrush::from_q_color(&QColor::from_rgb_3a(0, 0, 0)));
        // self.chart.set_background_brush(&QBrush::from_q_color(&QColor::from_rgb_3a(255, 255, 255)));
       ...
       let pen = QPen::new();
       pen.set_width_f(1.0);
       // pen.set_color(&QColor::from_rgb_3a(0, 205, 255));
       pen.set_color(&QColor::from_rgb_3a(0, 255, 0));
       self.series.set_pen(&pen);
       self.series.set_use_opengl_0a();
        ...
        *self.abort.lock().unwrap() = 0;
        ...
        'rx: for received in rx { // The receiver lives in the main Thread, GUI stuffs are performed here
            if *self.abort.lock().unwrap() == 0 {
                ...
                // QCoreApplication::process_events_1a(QFlags::from(ProcessEventsFlag::EventLoopExec));
            }
            else {
                break 'rx;
            }
...
fn main() {
    QApplication::init(|_| {
        ...
        todo_widget.show();
        unsafe {
            QCoreApplication::set_attribute_1a(ApplicationAttribute::AAUseOpenGLES);
            QApplication::exec()
        }
    })
}
